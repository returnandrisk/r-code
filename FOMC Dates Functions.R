...
# function to extract fomc data from 2009 onward
get.fomc.dates.from.2009 <- function() {
    print("Getting FOMC dates from 2009 onward...")
    ...
    for (i in 1:length(nodeset)) {
        ...
        tmp <- parse.dates.from.2009(days, months, year, statements, urls)
        fomcdates <- rbind(fomcdates, tmp)
    }
    fomcdates
}

# function to parse dates from 2009 onward
parse.dates.from.2009 <- function(days, months, year, statements, urls) {
    len <- length(days)
    # initialise dataframe
...
# function to extract fomc data from 1936 - 2008
get.fomc.dates.pre.2009 <- function(begyear = 1936, endyear = 2008) {
    ...
    for (year in seq(begyear, endyear)) { 
        ...
        for (i in 1:length(nodeset)) {
            ...
            tmp <- parse.dates.pre.2009(header, document, url)
            fomcdates <- rbind(fomcdates, tmp)
        }
    }
    fomcdates
...
# function to parse dates from 1936 - 2008
parse.dates.pre.2009 <- function(header, document, url) {
    # initialise dataframe
